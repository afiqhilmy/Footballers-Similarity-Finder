...
st.title("⚽ Football Player Similarity Finder")
...
with tab1:
    ...
    # -----------------------------
    # 2 Preprocessing (FROM NOTEBOOK)
    # -----------------------------
    data = data.drop("Rk", axis=1)

    data["Nation"] = data["Nation"].apply(
        lambda x: "".join(re.findall(r"[A-Z]", str(x)))
    )
    ...
    data["Born"] = data["Born"].astype("Int64")
    ...
    data["Born"] = data["Born"].fillna(data["Born"].median())
    ...
    def create_radar_chart(player1, player2):
        ...
        try:
            p1 = data[data["Player"] == player1][radar_stats].iloc[0].fillna(0)
            p2 = data[data["Player"] == player2][radar_stats].iloc[0].fillna(0)
        except:
            return None
